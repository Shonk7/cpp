...
i32 main() {
    ...
    // Ls beofre and Ws after
    vi prefixL(n + 1, 0);
    vi suffixW(n + 1, 0);

    range(i, 0, n) prefixL[i + 1] = prefixL[i] + (s[i] == L);
  
    rrange(i, n - 1, 0) suffixW[i] = suffixW[i + 1] + (s[i] == W);
    ...
    range(i, 0, n) {
        int k = min(prefixL[i], suffixW[i] - 1);
        if (k > best) {
            best = k;
            bestSplit = i;
        }
    ...
    // going through an creating output string
    string ans(n, '_');

    int needL = best;
    int i = 0;
    while (i < bestSplit && needL > 0) {
        if (s[i] == L) {
            ans[i] = s[i];
            needL--;
        }
