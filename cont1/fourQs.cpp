...
bool controlAll() {
    ...
    for (auto& [mr, mc] : marked) {
        bool covered = false;
        // check if any queens cover this sqaure
        for (auto& [qr, qc] : queens) {
            if (mr == qr || mc == qc || abs(qr - mr) == abs(qc - mc)) {
                covered = true;
                break;
            } 
        }
