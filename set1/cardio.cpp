...
i32 main() {
    ...
    vi suf(n + 1, LLONG_MIN);  // suf[i] = max_{j>=i}(m[j] - j)
    rrange(i, n - 1, 0) {
        suf[i] = max(suf[i + 1], m[i] - i);
    }

    int k = 0;
    int ans = 0;
    range(i, 0, n) {
        int need = (i + 1 < n ? i + 1 + suf[i + 1] : LLONG_MIN);
        bool can_reuse = (m[i] < k);
        bool safe_for_future = (i + 1 == n || k >= need);

        if (can_reuse && safe_for_future) {
            ans += k - m[i] - 1;
        } else {
            ans += k - m[i];
            k++;
