...
i32 main() {
    ...
    // get acitvity start and end
    range (i, 0, n) cin >> act[i].first >> act[i].second;

    multiset<int> rooms;
    // time end for room
    range(i, 0, k) rooms.insert(0);

    // sort end times in ascending orfer
    sort(all(act), [](const auto& a, const auto& b) {
        return a.second < b.second;
    });
    
    int count = 0;
    // choose otimal room that is the one with the first (last) end time < s
    for (auto& [s, e] : act) {
        // iterator to firs tposition s can be inserted in rooms
        auto it = rooms.lower_bound(s);
        // if at start than there is no valid room i.e. start time. is before all current end times
        if (it != rooms.begin()) {
            it--;
            // new room with ending time e, so rmeove old element and and a new one
            rooms.erase(it);
            rooms.insert(e);
            count++;
        }
